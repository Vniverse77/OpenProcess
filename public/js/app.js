...
function initSidebar(activePage) {
  document.querySelectorAll(".nav-item").forEach(item => { if (item.dataset.page === activePage) item.classList.add("active"); });
  const menuBtn = document.querySelector(".mobile-menu-btn");
  const sidebar = document.querySelector(".sidebar");
  if (menuBtn && sidebar) {
    menuBtn.addEventListener("click", () => sidebar.classList.toggle("open"));
    document.addEventListener("click", (e) => { if (window.innerWidth <= 768 && !sidebar.contains(e.target) && !menuBtn.contains(e.target)) sidebar.classList.remove("open"); });
  }
}

async function logout() { await api("/api/auth/logout", { method: "POST" }); window.location.href = "/login"; }

function formatMoney(n) { return new Intl.NumberFormat("tr-TR", { style: "currency", currency: "TRY", minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(n || 0); }
...
function formatDateTime(d) { if (!d) return "-"; return formatDate(d) + " " + formatTime(d); }
function timeAgo(d) {
  if (!d) return "";
...
function statusText(s) { return { waiting: "Bekliyor", in_progress: "İşlemde", completed: "Hazır", delivered: "Teslim Edildi", cancelled: "İptal" }[s] || s; }
function statusBadge(s) { return `<span class="badge badge-${s.replace("_","-")}"><span class="badge-dot"></span>${statusText(s)}</span>`; }
function categoryLabel(c) {
  const n = { yikama: "Yıkama", detailing: "Detailing", ic_temizlik: "İç Temizlik", aksesuar: "Aksesuar", genel: "Genel" };
